...
class Game:
    def __init__(self, game_id: str, chat_id: int, creator_id: int):
        ...
        self.first_round_done: bool = False
    ...
    def deal_first_round(self):
        ...
        for i, player in enumerate(self.players):
            ...
            player.cards = player.all_cards.copy()
    ...
    def play_card(self, user_id: int, card_index: int) -> Tuple[bool, Optional[Card], Optional[str]]:
        ...
        if not self.can_play_card(player, card, is_first_card):
            ...
            if valid_cards:
                return False, None, f"باید همخال بیاورید"
            else:
    ...
    def get_game_info_text(self) -> str:
        ...
        if self.state == "waiting":
            ...
            text += f"📊 دور اول: ۵ کارت\n\n"
            text += "👇 **روی دکمه کلیک کنید:**"
...
async def update_game_message(context, game):
    ...
    try:
        ...
        if game.state == "waiting":
            ...
            keyboard = [
                [
                    InlineKeyboardButton("♥️ دل", callback_data=f"trumppick_{game.game_id}_hearts"),
                    InlineKeyboardButton("♦️ خشت", callback_data=f"trumppick_{game.game_id}_diamonds")
                ],
                [
                    InlineKeyboardButton("♣️ گیشنیز", callback_data=f"trumppick_{game.game_id}_clubs"),
                    InlineKeyboardButton("♠️ پیک", callback_data=f"trumppick_{game.game_id}_spades")
                ]
            ]
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("join_"):
        ...
        try:
            keyboard = [[InlineKeyboardButton("✅ تایید و پیوستن", callback_data=f"verifyjoin_{game.game_id}")]]
    ...
    # ========== تایید و پیوستن ==========
    elif data.startswith("verifyjoin_"):
        game_id = data[11:]
        game = game_manager.get_game(game_id)
        ...
        if is_member:
            ...
            keyboard = [[
                ...
                InlineKeyboardButton("🔄 بررسی", callback_data=f"verifyjoin_{game.game_id}")
            ]]
        ...
        if game.start_game():
            ...
            for player in game.players:
                if player.cards:
                    ...
                    cards = "\n".join([f"{i+1}. {c.persian_name}" for i, c in enumerate(player.cards)])
...
def main():
    ...
    print("✅ ۵ کارت دور اول")
    print("✅ دکمه حکم: trumppick_")
