...
class Game:
    ...
    def add_player(self, player: Player) -> bool:
        ...
        # وقتی ۴ بازیکن کامل شدند، تیم‌بندی کن
        if len(self.players) == 4:
            self.assign_teams()
    ...
    def assign_teams(self):
        """تیم‌بندی بازیکنان بر اساس موقعیت در میز"""
        # بازیکنان روبه‌روی هم تیم هستند (موقعیت‌های 0 و 2 تیم 0، موقعیت‌های 1 و 3 تیم 1)
        for i, player in enumerate(self.players):
            player.team = i % 2  # 0 یا 1
    ...
    def get_teams_info(self) -> str:
        ...
        if team0_players:
            names = " و ".join([p.display_name for p in team0_players])
            text += f"• تیم ۱: {names}\n"
        
        if team1_players:
            names = " و ".join([p.display_name for p in team1_players])
            text += f"• تیم ۲: {names}\n"
    ...
    def get_game_info_text(self) -> str:
        ...
        if self.state == "waiting":
            ...
            # نمایش تیم‌ها اگر ۴ نفر کامل شدند
            if len(self.players) == 4:
            ...
            text += f"🎯 **انتخاب کننده حکم:** {chooser.display_name if chooser else '?'}\n"
            ...
            if len(self.players) == 4:
                ...
                text += f"🎯 **امتیاز تیم ۱:** {team0_score} دست\n"
                text += f"🎯 **امتیاز تیم ۲:** {team1_score} دست\n\n"
...
async def update_game_message(context: ContextTypes.DEFAULT_TYPE, game: Game):
    try:
        if game.state == "waiting":
            ...
            # FIX: اینجا callback_data باید درست باشه
            keyboard = [
                [
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "help_guide":
        ...
        # FIX: این قسمت باید درست کار کنه
        parts = data.split("_")
        ...
        if len(parts) >= 3:
            ...
            logger.info(f"انتخاب حکم: game_id={game_id}, suit={suit_str}, user={user.id}")
